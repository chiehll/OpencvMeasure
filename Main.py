...
def my_mouse_callback(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_MOUSEMOVE:  # When the mouse moved
        if (drawing == True):
            width = x - ix
            height = y - iy
    elif event == cv2.EVENT_LBUTTONDOWN:  # Left mouse pressed
        drawing = True
        ix = x
        iy = y
        width = 0
        height = 0
    elif event == cv2.EVENT_LBUTTONUP:  # When the mouse is released
        drawing = False
        complete_region = True
        kx = x
        ky = y

    box = [ix, iy, kx, ky, width, height]  # Selection range


# Binary portrait representation
...
def max_area(img_thresh, roi):
    ...
    if contours != []:
        ...
        roi_max = cv2.putText(roi_max, wh_hm, (xm + 2, ym + 20), cv2.FONT_HERSHEY_DUPLEX, 0.5, (255, 100, 0), 1,
                              cv2.LINE_AA)  # Depicting size
...
def main():
    ...
    while (True):
        ...
        frame_config = cv2.putText(frame_config, mode_text, (10, 40), cv2.FONT_HERSHEY_DUPLEX, 0.5, (0, 0, 200), 1,
                                   cv2.LINE_AA)
        frame_config = cv2.putText(frame_config, correction_text, (10, 60), cv2.FONT_HERSHEY_DUPLEX, 0.5, (0, 0, 200),
                                   1, cv2.LINE_AA)
        frame_config = cv2.putText(frame_config, threshold_text, (10, 80), cv2.FONT_HERSHEY_DUPLEX, 0.5, (0, 0, 200), 1,
                                   cv2.LINE_AA)
        frame_config = cv2.putText(frame_config, 'Line colour', (10, 100), cv2.FONT_HERSHEY_DUPLEX, 0.5, linecolour, 1,
                                   cv2.LINE_AA)

        frame_config = cv2.putText(frame_config, 'Temporarily saved : t', (10, 120), cv2.FONT_HERSHEY_DUPLEX, 0.5,
                                   (100, 0, 0), 1, cv2.LINE_AA)
        frame_config = cv2.putText(frame_config, 'Correction : c', (10, 140), cv2.FONT_HERSHEY_DUPLEX, 0.5, (100, 0, 0),
                                   1, cv2.LINE_AA)
        frame_config = cv2.putText(frame_config, 'Threshold +: x -: y', (10, 160), cv2.FONT_HERSHEY_DUPLEX, 0.5,
                                   (100, 0, 0), 1, cv2.LINE_AA)
        frame_config = cv2.putText(frame_config, 'Save : s', (10, 180), cv2.FONT_HERSHEY_DUPLEX, 0.5, (100, 0, 0), 1,
                                   cv2.LINE_AA)
        frame_config = cv2.putText(frame_config, 'Quit : q', (10, 200), cv2.FONT_HERSHEY_DUPLEX, 0.5, (100, 0, 0), 1,
                                   cv2.LINE_AA)
        ...
        if (drawing):  # Left click push
            ...
            if mode == 'Line':
                ...
                temp = cv2.putText(temp, width_height, (ix, iy - 10), cv2.FONT_HERSHEY_DUPLEX, 0.5, linecolour, 1,
                                   cv2.LINE_AA)  # Depicting length
            else:
                cv2.rectangle(temp, (ix, iy), (ix + width, iy + height), linecolour, 1)  # Draw rectangle
                width_height = str(int(height * correction)) + '-' + str(int(width * correction))
                temp = cv2.putText(temp, width_height, (ix, iy - 10), cv2.FONT_HERSHEY_DUPLEX, 0.5, linecolour, 1,
                                   cv2.LINE_AA)  # Depicting size

        if (complete_region):  # After the selection of the rectangle
            complete_region = False
            if (height != 0 and width != 0):
                ...
                if mode == 'Line':
                    ...
                    roi = orign[iy + 1:iy + height - 1, ix + 1:ix + width - 1]  # Cut selection from original image
